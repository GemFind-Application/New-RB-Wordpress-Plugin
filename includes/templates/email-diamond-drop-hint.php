...
if ( $role === 'sender' ) {
	...
	if ( ! empty( $phone_no ) ) {
		$hint_rows[] = [ 'label' => 'Your Phone:', 'value' => (string) $phone_no ];
	}
	$hint_rows[] = [ 'label' => 'Gift Deadline:', 'value' => (string) ( $deadline ?? '' ) ];
	$hint_rows[] = [ 'label' => 'Additional Message:', 'value' => (string) ( $hint_message ?? '' ) ];
	gemfindrb_email_info_table( $hint_rows );
