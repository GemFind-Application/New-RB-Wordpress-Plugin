...
const PortalPopup = ({
  ...
  const popupStyle = useMemo(() => {
    ...
    if (overlayColor && !overlayClassName?.includes('video-tryon')) {
      style.backgroundColor = overlayColor;
    }
    if (overlayClassName?.includes('video-tryon')) {
    ...
    style.opacity = 1;
    return style;
  }, [placement, overlayColor, overlayClassName, zIndex, relativeLayerRef?.current]);
  ...
  return (
    <Portal>
      <div
        className={`portalPopupOverlay ${overlayClassName}`.trim()}
        style={popupStyle}
...
export const Portal = ({ children, containerId = GEMFIND_PORTALS_ID }) => {
